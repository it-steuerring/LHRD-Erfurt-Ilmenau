...
@minLength(3)
@maxLength(255)
param domainFQDN string = 'exakt.local' //change domain here. Use simple domain like "contoso.local" instead of "ad.contoso.local". Always use .local as the top-level domain
...
// deallocation logic app parameters
// Webhook-Logic-App, mit der die letzte angemeldete Person alle Session Hosts
// per Desktop-Verknuepfung deallokieren kann.
param deallocateLogicAppName string = 'logic-avd-deallocate-prod-01'
param deallocateMaxRemainingActiveSessions int = 1 // Deallocate nur, wenn hoechstens so viele AKTIVE Sitzungen verbleiben
param deallocateLogoffRemainingSessions bool = true // Verbleibende Sitzung vor dem Deallocate sauber abmelden
param deallocateLogoffWaitSeconds int = 60 // Wartezeit in Sekunden zwischen Logoff und Deallocate
param deployDeallocateShortcut bool = true // Desktop-Verknuepfung auf den Session Hosts anlegen
param deallocateShortcutName string = 'AVD herunterfahren'
...
// Deploy the webhook logic app that deallocates the session hosts.
// Wird vor den Session Hosts deployt, weil deren Desktop-Verknuepfung die
// Callback-URL dieses Workflows braucht.
module avdDeallocateLogicApp 'modules/avdDeallocateLogicApp.bicep' = {
  name: 'avdDeallocateLogicApp'
  params: {
    location: location
    logicAppName: deallocateLogicAppName
    hostPoolResourceId: avdHostPool.outputs.hostPoolId
    maxRemainingActiveSessions: deallocateMaxRemainingActiveSessions
    logoffRemainingSessions: deallocateLogoffRemainingSessions
    logoffWaitSeconds: deallocateLogoffWaitSeconds
  }
}
...
module avdSessionHosts 'modules/avdSessionHosts.bicep' = [for i in range(0, avdSessionHostCount): {
  ...
  params: {
    ...
    shutdownWebhookUrl: avdDeallocateLogicApp.outputs.webhookUrl
    deployShutdownShortcut: deployDeallocateShortcut
    shortcutName: deallocateShortcutName
  }
}]
...
// ---------- OUTPUTS ----------
// Wird gebraucht, um die Desktop-Verknuepfung nachtraeglich oder per GPO zu verteilen
// (scripts/New-AvdShutdownShortcut.ps1 holt die URL alternativ direkt per Azure REST).
// Achtung: die URL enthaelt eine SAS-Signatur, die den Workflow ohne weitere
// Authentifizierung ausloest, und landet damit in der Deployment-History.
@description('Callback-URL des AVD-Deallocate-Webhooks')
output deallocateWebhookUrl string = avdDeallocateLogicApp.outputs.webhookUrl
