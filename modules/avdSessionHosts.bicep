...
@description('Callback-URL des AVD-Deallocate-Workflows fuer die Desktop-Verknuepfung. Wird nur verwendet, wenn deployShutdownShortcut true ist.')
@secure()
param shutdownWebhookUrl string = ''

@description('Desktop-Verknuepfung anlegen. Wird als normaler Bool gefuehrt, damit die Bedingung der Ressource nicht von einem @secure()-Parameter abhaengt.')
param deployShutdownShortcut bool = false

@description('Anzeigename der Desktop-Verknuepfung zum Herunterfahren der Session Hosts')
...
// 3) Managed Run Command, das die Desktop-Verknuepfung zum Deallokieren der
// Session Hosts auf den Public Desktop legt. Bewusst ein Run Command und keine
// zweite CustomScriptExtension: dieser Extension-Handler ist pro Windows-VM nur
// einmal moeglich und oben schon von "RegisterSessionHost" belegt.
resource shutdownShortcut 'Microsoft.Compute/virtualMachines/runCommands@2024-07-01' = if (deployShutdownShortcut) {
  parent: vm
  name: 'CreateAvdShutdownShortcut'
  location: location
  dependsOn: [ registerHost ]
  properties: {
    asyncExecution: false
    timeoutInSeconds: 300
    // Ein fehlgeschlagenes Icon darf das AVD-Deployment nicht scheitern lassen.
